...
const getAllGiftCategories = async (req: Request, res: Response): Promise<void> => {
  try {
    const giftCategories: Gift_Categories[] = await knex<Gift_Categories>(
      "gift_categories"
    ...
    res
      .status(400)
      .send(
        `Error fetching gift categories from gift_categories table: ${
          error.message || error
        }`
      );
  }
...
export { getAllGifts, getAllGiftCategories };
